...
export function ChatPageClient({ threadId }: { threadId?: string }) {
  ...
  if (isAuthLoading || !isAuthenticated) {
    return <div className="h-[32rem] animate-pulse rounded-2xl bg-[#f2f5f1]" />
  }

  return (
    <div className="grid h-[36rem] overflow-hidden rounded-2xl border border-[#e1e7df] bg-white shadow-[0_14px_40px_rgba(21,45,25,0.04)] lg:grid-cols-[22rem_minmax(0,1fr)]">
      <div
        className={cn(
          'min-h-0 border-[#eaefe8] lg:border-r',
