...
import { moderationApi, type PendingListing } from '@/lib/api/moderation'
...
export function PendingListingsPanel() {
  ...
  function invalidate() {
    queryClient.invalidateQueries({ queryKey: ['moderation', 'pending-listings'] })
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) => moderationApi.rejectListing(id, reason),
    onSuccess: () => {
  ...
  if (!data || data.items.length === 0) {
    return <EmptyState title={t('listingsEmptyTitle')} description={t('listingsEmptyDescription')} />
  }
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-[#5f6c61]">
                  {listing.product?.name ?? '—'} · {sellerName} · {formatDate(listing.createdAt)}
                </p>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <Button
              type="button"
              onClick={() => rejectTarget && rejectMutation.mutate({ id: rejectTarget.id, reason: reason.trim() })}
